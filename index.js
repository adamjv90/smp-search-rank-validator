...
index.search(process.argv[2], {
    ...
    each(contents.hits, function (row) {
        table.push([row.objectID, row.ym.substring(4, 6) + '/' + row.ym.substring(2, 4), row.gid, row.isImageLbb ? figures.tick : figures.cross, tagsMatchingQuery(row._highlightResult.tags3).join(''), tagsMatchingQuery(row._highlightResult.tags2).join(''), tagsMatchingQuery(row._highlightResult.tags1).join(''), tagsMatchingQuery(row._highlightResult.etagsA).join(''), tagsMatchingQuery(row._highlightResult.etagsB).join('')]);
    });
